...
public class LaboratoryController {
	...
	@GetMapping("/labvalid")
	public String LaboratoryValidation(Model model, @ModelAttribute("laboratory")  @RequestParam("givenCaptcha") String givenCaptcha,@RequestParam("userCaptcha") String userCaptcha,
			HttpServletRequest request,Laboratory objlaboratory) {
		HttpSession session = request.getSession();
		if (!givenCaptcha.equals(userCaptcha)) {
			session.setAttribute("indexmsg","logininvalidcaptcha");
			return "redirect:/";
		}
		String str = VcareUtilies.getEncryptSecurePassword(objlaboratory.getPassword(), applicationName);
		Laboratory lab=laboratoryRepository.findByMailIdIgnoreCaseAndPassword(objlaboratory.getEmail(), str);
		System.err.println("====="+objlaboratory.getEmail());
		if (lab != null) {
